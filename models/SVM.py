"""
ENGG2112 - Pluvial Flood Susceptibility: SVM Model
====================================================
...
# =========================================================
# SVM requires StandardScaler: the maximum margin hyperplane is sensitive to
# feature scale, so unscaled features with large ranges would dominate the
# kernel distance calculations.
# probability=True enables predict_proba for ROC curve computation via
# Platt scaling, at a small additional compute cost.
pipeline = Pipeline([
...
# C:     regularisation parameter; small C = wider margin with more misclassifications
#        (high bias), large C = narrower margin fitting training data tightly (high variance)
# gamma: controls the influence radius of each training point;
#        'scale' = 1 / (n_features * X.var()) is a safe data-driven default,
#        'auto'  = 1 / n_features ignores feature variance
param_grid = {
...
ax.set_title(f"SVM Confusion Matrix – {case_name}")
plt.tight_layout()
plt.show()

# -- Permutation importance --
# SVMs have no native feature importance, so permutation importance is used:
# each feature is shuffled on the test set and the accuracy drop is recorded.
...
ax.set_title(f"SVM Permutation Importance – {case_name}")
plt.tight_layout()
plt.show()
...
ax.set_title(f"SVM ROC Curves – {case_name}")
